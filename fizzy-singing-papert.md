...
Dieser erste Ansatz (`EVENT_CALL_SERVICE`) wurde committet, dann aber als **grundlegend unzureichend für den nativen „+"-Button erkannt** — siehe nächstes Update.

## Update 2026-09-09 (Fortsetzung 5): Architekturfehler im Reminder-Backfill gefunden und durch Polling ersetzt

Nach dem Push hat der Nutzer live über den nativen „+"-Button einen Termin angelegt („test", 9.9. 14:00–15:00 im „Privat"-Kalender) — **kein Reminder kam an**, obwohl kein Fehler beim Anlegen auftrat und die Standard-Erinnerungsmethode korrekt auf „Popup"/60 Min. konfiguriert war. Erste Vermutung (Termin liegt im falschen, nicht verwalteten Kalender) war falsch und wurde durch Nachschauen im Termin-Detail-Dialog widerlegt — der Termin lag korrekt in „Privat".

**Root Cause per Quellcode-Lektüre gefunden, nicht geraten**: `home-assistant/frontend`s `src/data/calendar.ts` zeigt, dass der native Termin-Dialog `hass.callWS({ type: "calendar/event/create", ... })` aufruft — einen direkten WebSocket-Befehl, **keinen** Service-Call. `EVENT_CALL_SERVICE` (worauf der komplette bisherige Mechanismus lauschte) feuert für diesen Pfad **nie**. Jeder bisherige „erfolgreiche" Test dieser Session lief über ein manuell simuliertes `call_service`-Event (Entwicklerwerkzeuge → Ereignisse) — nie über den echten Button. Der Mechanismus hat für seinen eigentlichen Zweck von Anfang an nicht funktioniert; nur die synthetischen Tests haben das verdeckt.

Nebenbefund beim Debuggen: Home Assistants „Protokolle"-Systemlog hält nur WARNING/ERROR im Speicher, unabhängig von aktivierter Debug-Protokollierung — INFO/DEBUG-Zeilen landen zwar im echten Log-Stream, aber nie in dieser UI. Sichtbar nur über das „Terminal"-Add-on (`ha core logs`), mitgeschnitten in eine Datei (`ha core logs -f > /tmp/x.txt &`), da die extrem gesprächige `zendure_ha`-Integration den Log-Ringpuffer sonst binnen Minuten flutet. Außerdem: die „Debug-Protokollierung aktiviert"-Einstellung pro Integration übersteht einen `ha core restart` **nicht** und muss danach manuell erneut aktiviert werden.

**Fix, nach Rücksprache mit dem Nutzer** (Frage: Event-Listener ganz durch Polling ersetzen, oder beides kombinieren? → „Beides kombinieren"): Der `EVENT_CALL_SERVICE`-Listener (inkl. seines Retry-mit-Backoff-Mechanismus gegen den unten beschriebenen Core-`caldav`-Timeout) bleibt bestehen für alles, was den Service tatsächlich aufruft (Automationen/Skripte mit `calendar.create_event` als Action) — reagiert sofort, ohne auf den nächsten Poll zu warten. Neu ergänzt: ein periodischer Poller (`async_track_time_interval`, alle 60 s), der für jeden konfigurierten Kalender per `date_search` im Fenster `[jetzt−1 Tag, jetzt+365 Tage]` nach Events ohne `VALARM` sucht — das deckt sowohl den nativen „+"-Button als auch (als Bonus, vorher als „nicht abdeckbar" dokumentiert) einen direkt in der iOS-Kalender-App angelegten Termin ab, sobald er per iCloud-Sync auf dem Server liegt.

**Sicherheitsmechanismus gegen Überraschungen**: Ein reiner „patch jedes Event ohne VALARM"-Poller würde beim allerersten Lauf jeden bereits bestehenden, bewusst reminder-losen Alt-Termin verändern. Neue Klasse `SeenEventsTracker` (`seen_events.py`, `Store`-backed wie `ReminderScheduler`) persistiert pro Kalender die Menge bereits gesehener Event-UIDs über Neustarts hinweg. Beim allerersten Poll eines Kalenders (`has_baseline() == False`) werden nur die aktuellen UIDs als Baseline gespeichert, **ohne** zu patchen (`skip_backfill=True`); erst ab dem zweiten Poll gelten neu auftauchende UIDs als „neu" und werden bei fehlendem `VALARM` gepatcht. Einmal gesehene Events werden nie wieder angefasst, selbst wenn ein Nutzer später den Reminder manuell entfernt.

Neue Methode `CalDavCalendarTarget.async_backfill_new_events()`/`_backfill_new_events()` in `caldav_target.py` implementiert das Poll-und-Patch pro Kalender; vier neue Unit-Tests (`test_poll_backfills_a_new_reminder_less_event`, `test_poll_skips_an_already_known_uid`, `test_poll_with_skip_backfill_only_collects_uids`, plus die drei bereits vorhandenen Backfill-Tests) decken Treffer, bereits bekannte UID und den Baseline-Skip ab.

**Live end-to-end verifiziert, diesmal über den echten nativen Button** (nicht mehr synthetisch): Termin „Poll Verify Test" per „+"-Button im „Privat"-Kalender angelegt, nach dem nächsten 60-Sekunden-Poll erschien automatisch, ohne jedes manuelle Eingreifen:
```
INFO [custom_components.calendar_bridge.caldav_target] Backfilled a popup reminder onto 'Poll Verify Test' (poll)
```
Das ist die erste echte Bestätigung des ursprünglich vom Nutzer gewünschten Verhaltens („kannst du nicht sogar darauf reagieren").
...
## Update 2026-09-09 (Fortsetzung 7): Poller-Verifikation an einem echten neuen Termin

Nutzer legte einen neuen Termin („Test Matthias", Kalender „Privat", 16:00–17:00) über den nativen „+"-Button an und fragte direkt, ob der Trigger ausgelöst hat. Ein frischer `ha core logs -f`-Mitschnitt (gestartet nach Termin-Erstellung) zeigte zunächst **keine** Backfill-Zeile — Grund war reine Timing-Lücke: der Poll, der den Termin erfasst hatte, lief vermutlich knapp vor Mitschnittstart, die UID war beim Nachprüfen bereits als „bekannt" gespeichert (kein erneutes Anfassen laut `SeenEventsTracker`-Design).

Statt das ohne Beweis zu behaupten, direkte Verifikation per temporärer Code-Instrumentierung: kurzzeitig `print()`-Zeilen in `_async_poll_for_new_events` und `_backfill_new_events` eingefügt (Deploy → Neustart → Log geprüft → sofort wieder entfernt, erneut deployed und syntaktisch/mit `ast.parse` gegengeprüft). Ergebnis: Poll-Callback läuft nachweislich alle 60 s; für die UID von „Test Matthias" zeigte die Diagnosezeile `bereits bekannt=True, VALARM-Anzahl=1` — der Termin hat also tatsächlich einen von diesem Poller gesetzten Reminder, das System hat korrekt funktioniert. Kein Bug, nur ein zu später Log-Mitschnitt-Start.

## Update 2026-09-09 (Fortsetzung 8): Phase 2 — Google Calendar als zweites Backend

Auf Wunsch des Nutzers („mach 3" nach Rückfrage zu den offenen Punkten) Phase 2 aus dem ursprünglichen Phasenplan umgesetzt. Abweichend vom in der Einleitung skizzierten Ansatz („eigene Application Credentials unter der `calendar_bridge`-Domain registrieren, Risiko der Wiederverwendung empirisch prüfen") wählte der Nutzer explizit die einfachere Variante: **„das solltest du auch aus HA einfach nutzen ohne zusätzliche Eingabe"** — Calendar Bridge registriert also keine eigenen OAuth-Credentials und zeigt keinen eigenen Consent-Screen, sondern leiht sich live die `OAuth2Session` eines bereits konfigurierten Core-`google`-Kontos.

**Technischer Nachweis vor der Implementierung** (Quellcode-Lektüre von `home-assistant/core`s `google`-Integration, nicht Vermutung): `homeassistant/components/google/__init__.py` zeigt exakt das Muster `implementation = await config_entry_oauth2_flow.async_get_config_entry_implementation(hass, entry); session = config_entry_oauth2_flow.OAuth2Session(hass, entry, implementation)`. Das lässt sich unverändert auf eine **fremde** (nicht die eigene) ConfigEntry anwenden — nichts daran ist an die Domain `google` gebunden, nur der übergebene `entry` muss zu dieser Domain gehören. Damit entfällt der ursprünglich befürchtete Risiko-Punkt komplett: keine zweite Konsens-Runde, keine eigene Client-ID nötig.

Als API-Client kommt `gcal_sync` (PyPI, von Nabu Casa/`allenporter` gepflegt, `py.typed`) zum Einsatz — dieselbe Bibliothek, die Core's eigene `google`-Integration nutzt (`GoogleCalendarService`, pydantic-Modelle `Event`/`Reminders`/`ReminderOverride`). Wichtiger Unterschied zu CalDAV/VALARM: Ein Google-Event hat *immer* ein `reminders`-Objekt; `useDefault: true` (der Default) heißt nur „erbt die Kalender-Standard-Erinnerung", was auch „gar keine" bedeuten kann. Ein Event gilt hier nur dann als „hat bereits eine Erinnerung", wenn `reminders.overrides` nicht leer ist — Analogon zu „hat ein VALARM" bei CalDAV.

**Neue/geänderte Dateien**:
- `target.py`: `CalendarTarget`-Protocol um `async_backfill_reminder`/`async_backfill_new_events` erweitert (vorher nur implizit über Duck-Typing genutzt); `as_utc()` und `all_day_bounds()` aus `caldav_target.py` hierher verschoben, damit beide Backends dieselbe Datums-Normalisierung teilen.
- `google_target.py` (neu): `GoogleCalendarTarget` mit `async_create_event`/`async_backfill_reminder`/`async_backfill_new_events` — spiegelt exakt die CalDAV-Zielklasse, nur gegen die Google-REST-API statt CalDAV/ICS. `async_list_writable_calendars()` filtert per `accessRole` schreibgeschützte Kalender (u. a. Googles automatischen „Geburtstage"-Kalender) heraus — genau das im ursprünglichen Risiko-Abschnitt notierte Problem.
- `config_flow.py`: neuer Menüpunkt „Google-Kalender verwenden" (nur sichtbar, wenn mind. ein Core-`google`-Konto existiert; bei genau einem Konto wird es automatisch gewählt, ganz ohne Zwischenschritt — passend zur „ohne zusätzliche Eingabe"-Vorgabe). Kalenderauswahl-Subentry-Flow erlaubt bei Google zusätzlich `email` als Erinnerungsmethode (bei CalDAV weiterhin nur `popup`/`none`, siehe Fortsetzung 3).
- `__init__.py`: `async_setup_entry` erkennt am Vorhandensein von `CONF_GOOGLE_ENTRY_ID` vs. `CONF_URL` in `entry.data`, welches Backend instanziiert wird — Listener und Poller selbst blieben unverändert, da sie schon immer nur generisch gegen das `CalendarTarget`-Interface programmiert waren.
- `manifest.json`: `requirements` um `gcal-sync==9.1.1` ergänzt, `after_dependencies: ["google"]` (weiche Abhängigkeit — Calendar Bridge bleibt auch ohne konfiguriertes Google-Konto nutzbar, rein CalDAV), `iot_class` von `cloud_push` auf `cloud_polling` korrigiert (stimmt jetzt mit dem seit Fortsetzung 5 tatsächlich vorhandenen Poller überein).
- `tests/test_google_target.py` (neu): 8 Tests nach demselben Muster wie `test_caldav_target.py` — Event-Erstellung inkl. Reminder-Payload, `calendar_not_found` bei 404, Backfill-Treffer/-Skip (bereits vorhandene Erinnerung, falscher Titel), Poll-Verhalten (neue UID, bekannte UID, Baseline-Skip). OAuth/Session-Aufbau wird per `patch.object(target, "_async_service", ...)` komplett umgangen — die Tests prüfen nur die Event-/Reminder-Logik, keine echte HA-OAuth-Infrastruktur.

**Lokale Verifikationsgrenzen** (wie gehabt, siehe Fortsetzung 3): `pytest` kann auf diesem Windows-Rechner grundsätzlich nicht laufen (Socket-Guard blockiert `ProactorEventLoop`), betrifft auch die neuen Google-Tests identisch zu den bestehenden CalDAV-Tests — kein neues Problem. `ruff check .`, `ruff format --check .` und `mypy --strict custom_components/calendar_bridge` liefen lokal sauber durch; die verbleibenden mypy-Meldungen sind ausschließlich die bereits bekannten, durch den veralteten lokalen HA-Stub (2025.1.4, keine Subentries) verursachten Fehlalarme, keine neuen. `gcal-sync==9.1.1` selbst ließ sich lokal nicht installieren (verlangt Python ≥3.13, lokales venv hat 3.12) — Typprüfung stattdessen gegen die letzte pip-installierbare Version (8.0.0) laufen lassen und dabei zwei echte Versions-Unterschiede gefunden und korrigiert (ältere `gcal_sync`-Modelle akzeptieren nur Alias-Namen wie `dateTime`/`useDefault`/`calendarId` statt der Python-Feldnamen) — durchgängig auf Alias-Konstruktion umgestellt, was unabhängig von der `populate_by_name`-Einstellung in jeder Version funktioniert. Echter End-to-End-Test gegen ein reales Google-Konto steht noch aus (braucht ein tatsächlich konfiguriertes Core-`google`-Konto auf dem Ziel-System).
